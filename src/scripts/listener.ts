...
// if (!PICASSO_TOKEN) {
//     throw new Error("CRITICAL ERROR: PICASSO_TOKEN is not defined in the .env file.");
// }
...
//         //       headers: { 'x-picasso-token': PICASSO_TOKEN }
//         //     }),
//         //     3, 1000, "fetch-details"
//         //   );
...
//           const detailsResponse = await retryRequest(
//             // The axios.get call now includes the 'x-picasso-token' header.
//             () => axios.get(getDetailsUrl, {
//               headers: { 'x-picasso-token': PICASSO_TOKEN }
//             }),
//             3, 1000, "fetch-details"
...
//         //     () => axios.put(updateUrl, { new_status: statusToSend }, {
//         //         headers: {
//         //             'x-picasso-token': PICASSO_TOKEN
//         //         }
//         //     }),
//         //     3, 1000, "update-status"
//         // );
...
//               2, 2000, "flask-api"
//             );
//             if (flaskResponse) finalStatus = "Completed";
//           } catch (flaskErr: any) {
//             logger.error(`❌ Flask API error: ${flaskErr.message}`);
//           }
//         } catch (jobErr: any) {
...
//           await retryRequest(
//             // The axios.put call also includes the 'x-picasso-token' header.
//             () => axios.put(updateUrl, { new_status: finalStatus }, {
//               headers: { 'x-picasso-token': PICASSO_TOKEN }
//             }),
//             3, 1000, "update-status"
//           );
...
//           logger.info(`📦 Updated request ${job.request_id} -> ${finalStatus}`);
...
// Get the secret token from the environment. This is our "master key".
const PICASSO_TOKEN = process.env.PICASSO_TOKEN;
...
const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));
const retryRequest = async (fn: () => Promise<any>, retries = 3, delayMs = 1000, context = "unknown"): Promise<any> => {
    // This retry logic is correct and does not need to change.
    let attempt = 0;
    while (attempt < retries) {
        try {
            return await fn();
            ...
            logger.warn(`⚠️ [${context}] Attempt ${attempt} failed: ${errorMessage}. Retrying in ${delayMs}ms...`);
            if (attempt === retries) {
                logger.error(`❌ [${context}] All ${retries} attempts failed.`);
  ...
  while (true) {
    try {
      const { Messages } = await sqsClient.send(new ReceiveMessageCommand({
          QueueUrl: queueUrl, MaxNumberOfMessages: 1, WaitTimeSeconds: 20, VisibilityTimeout: 60,
      }));
      ...
      for (const message of Messages) {
        ...
        try {
          ...
          if (!request_id || !campaign_id) {
            throw new Error("Invalid SQS message: missing request_id or campaign_id");
          }

          // --- THIS IS THE FIX ---
          // Step 1: Fetch request details from backend, INCLUDING the secret token.
          const getDetailsUrl = `${backendBaseUrl}/api/v1/campaign/${campaign_id}/image/requests/${request_id}/get`;
          logger.info(`🔍 Fetching job details from: ${getDetailsUrl}`);

          const detailsResponse = await retryRequest(
            // The axios.get call now correctly includes the 'x-picasso-token' header.
            () => axios.get(getDetailsUrl, {
              headers: { 'x-picasso-token': PICASSO_TOKEN }
            }),
            3, 1000, "fetch-details"
          ...
          if (!detailsResponse || !detailsResponse.data) {
            throw new Error(`No data found for request_id ${request_id}`);
          }
          const fullJobDetails = detailsResponse.data;
        ...
        if (job.request_id && job.campaign_id) {
          ...
          await retryRequest(
            // The axios.put call also correctly includes the 'x-picasso-token' header.
            () => axios.put(updateUrl, { new_status: finalStatus }, {
              headers: { 'x-picasso-token': PICASSO_TOKEN }
            }),
            3, 1000, "update-status"
        ...
        // to prevent infinite retry loops for unfixable errors.
        if (message.ReceiptHandle) {
