...
if (!queueName) {
  throw new Error("CRITICAL ERROR: Environment variable 'IMAGE_GEN_QUEUE' is not defined.");
}
if (!backendBaseUrl) {
  throw new Error("CRITICAL ERROR: Environment variable 'BACKEND_API_BASE_URL' is not defined.");
}
if (!flaskApiEndpoint) {
  throw new Error("CRITICAL ERROR: Environment variable 'FLASK_API_ENDPOINT' is not defined.");
}
if (!PICASSO_TOKEN) {
  throw new Error("CRITICAL ERROR: Environment variable 'PICASSO_TOKEN' is not defined.");
}
  ...
  try {
    ...
    try {
      ...
      const detailsResponse = await retryRequest(
        () => {
        console.log('headers:', { 'x-picasso-auth': PICASSO_TOKEN });
      ...
      try {
        ...
        logger.error(`❌ Error sending job to Flask API: ${flaskErr.message}`);
      }
      } catch (detailsErr: any) {
      logger.error(`❌ Error fetching job details: ${detailsErr.message}`);
      }
    } catch (jobErr: any) {
    ...
    try {
      ...
      logger.error(`❌ Failed to update status for request ${job.request_id}: ${updateErr.message}`);
    }
    ...
    try {
      ...
      logger.error(`❌ Failed to delete message from SQS: ${deleteErr.message}`);
    }
    }
  } catch (pollErr: any) {
