...
describe('TemplateService', () => {
  ...
  describe('save', () => {
    ...
    it('rejects duplicates within the same vertical', async () => {
      ...
      ).rejects.toThrow('templatename already exists under this vertical, please use another name.');
    ...
    it('rejects when the parent vertical cannot be found', async () => {
      mockedTemplateDAO.list.mockResolvedValueOnce([]).mockResolvedValueOnce([]);
      mockedVerticalDAO.findById.mockResolvedValueOnce(null);

      await expect(
        TemplateService.save({ templatename: 'Email', verticalId: 1 })
      ).rejects.toThrow("Cannot save template because Vertical with ID '1' does not exist.");
    });
    ...
    it('saves a template when validation passes', async () => {
      ...
      const result = await TemplateService.save({ templatename: 'Email', verticalId: 1 });
      ...
      expect(result).toEqual({ templateId: 5, templatename: 'Email', verticalId: 1 });
    });
  });
